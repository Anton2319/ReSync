...
def cordstooffset(latitude, longitude):
    try:
        return convertjson(Get("http://api.timezonedb.com/v2.1/get-time-zone?key=SI3CNFOCL0F3&format=json&by=position&lat=" + str(latitude) + "&lng=" + str(longitude)))["gmtOffset"]
    except: return 0
...
def messagecust(msg="", peer_id=None, attachment="", keyboard="", intent="default", disable_mentions=0, dont_parse=1, reply_to=0):
    ...
    if msg != "":
        ...
        vk.messages.send(random_id=rid(), peer_id=peer_id, message=message, attachment=attachment, keyboard=keyboard, intent=intent, disable_mentions=disable_mentions, dont_parse_links=dont_parse, reply_to=reply_to)
